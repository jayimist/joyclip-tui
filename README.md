# JoyClip-TUI
A generic Joyful Clipboard TUI program. (You may find bugs n' stuff in this, expect grains of salt.)

## CLI Arguments
  - help / h - Shows more info of JoyClip-TUI
  - version / v - Shows the version of JoyClip-TUI

## How to use the TUI
  - You are in a list of text clips, navigate with the pointer and interact with a clip. Copy, delete, add, search for clips.
  - Controls
    - W - Move the pointer up.
    - S - Move the pointer down.
    - Q - Copies a clip.
    - A - Adds a clip manually.
    - E - Deletes clip.
    - D - Searches for a clip. (NOT IMPLEMENTED)
    - Ctrl+C - Exits the program.
  - In search mode to clear searching go into the search bar and press enter to default to all clips shown.

## How to run this?
Not gonna lie, I don't know how this software distribution is suppose to work.

Guess you can get the project in `./dist/joyclip-tui/` with the executable.
